...
#include "kernel_netlink_hunter_ath10k.c"	
...
static int ath10k_fw_stats_open(struct inode *inode, struct file *file)
{
	...
	mutex_unlock(&ar->conf_mutex);
		...
		printk(KERN_DEBUG "[ath10k][debug.c][ath10k_fw_stats_open]\n");	
	int a  = myinit_module();	
	temp(inode);
	printk(KERN_DEBUG "done calling the temp-ath10k function\n");
...
{
	printk(KERN_DEBUG "[ath10k][debug.c][ath10k_fw_stats_release]\n");	
				    ...
				    size_t count, loff_t *ppos)
{
	...
	printk(KERN_DEBUG "[ath10k][debug.c][ath10k_fw_stats_read1]\n");
	return simple_read_from_buffer(user_buf, count, ppos, buf, len);
	printk(KERN_DEBUG "[ath10k][debug.c][ath10k_fw_stats_read2]\n");
}
				     ...
				     size_t count, loff_t *ppos)
{
	...
	mutex_unlock(&ar->conf_mutex);
	printk(KERN_DEBUG "[ath10k][debug.c][ath10k_reg_addr_write]%x\n", reg_addr);
	return count;
}
				     ...
				     size_t count, loff_t *ppos)
{
	...
	reg_addr = ar->debug.reg_addr;
	printk(KERN_DEBUG "[ath10k][debug.c][ath10k_reg_value_read]%08x\n", reg_addr);

	reg_val = ath10k_hif_read32(ar, reg_addr);
	printk(KERN_DEBUG "[ath10k][debug.c][ath10k_reg_value_read]%x\n", reg_val);
	len = scnprintf(buf, sizeof(buf), "0x%08x:0x%08x\n", reg_addr, reg_val);

	ret = simple_read_from_buffer(user_buf, count, ppos, buf, len);
	if(jj==00)
	{
	printk(KERN_DEBUG "[ath10k][debug.c][ath10k_reg_value_read]\n");	
	// int a  = myinit_module();	
	// temp(file);
	printk(KERN_DEBUG "done calling the temp-ath10k function\n");
	jj== 11;
