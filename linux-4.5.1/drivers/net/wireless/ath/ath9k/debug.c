...
static int read_file_queues(struct seq_file *file, void *data)
{
	...
	printk(KERN_DEBUG "[ath9k][debug.c][queueus]\n");
	int b  = NETLINK_TEST2_init();
	temp2(file);
	printk(KERN_DEBUG "done calling the temp2 function\n");
...
static int open_file_regdump(struct inode *inode, struct file *file)
{
	...
	ath9k_ps_wakeup(sc);
		...
		int a  = myinit_module();	
			temp(inode);
	printk(KERN_DEBUG "done calling the temp function\n");
...
static int read_file_dump_nfcal(struct seq_file *file, void *data)
{
    ...
    char *found;
    char msg[] = "1 1 0x8098 0x8098 0x8098";
